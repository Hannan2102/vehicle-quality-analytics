...
- The **PP100 (complaints per 100 units)** KPI uses the same problems-per-100-units format applied industry-wide for vehicle quality benchmarking (e.g. J.D. Power's Initial Quality Study, for consumer new-vehicle surveys) — a direct hit on the "Customer Satisfaction" half of the role. This dataset tracks defect-linked complaints from B2B commercial fleets rather than a consumer ownership survey, so the scale is calibrated to that narrower population and isn't meant to be read against published consumer IQS scores directly.
